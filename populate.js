...
states.forEach((currState) => {

  politician.find({state: currState.abbreviation}, function(err, members){

    if (err) console.log(err)

    members.forEach((member) => {

      politician.updateOne({_id : member._id }, {state: currState.full}, {multi:true}, function(err, done){

        if(err) console.log(err)

        console.log("Changed state name for " + member.first_name + " " + member.last_name)
      });
